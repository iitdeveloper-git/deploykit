...
Thank you for your interest in contributing to `iitdeveloper-git/shared-workflows`! We welcome contributions, bug reports, workflow additions, and improvements from the open source community.
...
- **Least Privilege:** Always specify explicit `permissions` on reusable workflows.
...
### 2. Testing Locally
- **Unit Tests:** Run Python unit tests for actions/scripts:
  ```bash
...
- **Linting & Validation:**
  ...
  - Ensure Python scripts adhere to standard PEP 8 formatting.

---

## Submitting Pull Requests

1. Commit your changes using conventional commit messages (e.g., `feat(telegram): add custom header support`, `fix(security): sanitize markdown links`).
2. Push your topic branch to your fork.
3. Open a Pull Request against the `main` branch of `iitdeveloper-git/shared-workflows`.
...
We also maintain moving major tags (e.g., `v1`) pointing to the latest stable release of that major version.
